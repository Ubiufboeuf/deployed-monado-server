...
import type { Video, VideoDescription } from '../types/videoTypes'

export function getVideoById (id: string): Video | undefined {
  return vault.entries.get(id)?.info
}

export function getVideoDescription (id: string): VideoDescription | undefined {
  const entry = vault.entries.get(id)
  if (!entry) return
